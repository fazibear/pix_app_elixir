...
void set_realtime()
{
    ...
    sp.sched_priority = 60; //sched_get_priority_max(SCHED_FIFO);
...
uint8_t matrix[BRIGHTNESS][LINES][PER_LINE] = {
  {
    {
...
void set_dot(int x, int y, int r, int g, int b)
{
    ...
    l = x / 8;
    for(brightness = 0; brightness < BRIGHTNESS; brightness++) {
      t = matrix[brightness][y][l];
      if(g) {
        t |= 1 << p;
      } else {
        t &= ~(1 << p);
      }
      matrix[brightness][y][l] = t;
    }
...
void draw()
{
    uint8_t brightness, line, pos, bit;
      ...
      for(brightness = 0; brightness < BRIGHTNESS; brightness++) {
        for(line = 0; line < LINES; line++) {
          ...
          for(pos = 0; pos < PER_LINE; pos++) {
            for (bit = 0; bit < 8; bit++)  {
              SET_GPIO(SDI, !!(matrix[brightness][line][pos] & (1 << (7 - bit))));
              SET_GPIO(CLK, 1);
              SET_GPIO(CLK, 0);
            }
          }
